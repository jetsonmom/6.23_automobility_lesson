# 📘 AI 학습 정리       

## 1. About GitHub, Markdown, Colab      
- [GitHub 사용법](github-guide.md)
- [Markdown 문법](markdown-guide.md)     
- [Colab 기초](colab-guide.md)

## 2. About Python3
- [Python basic](python3.md)

## 3. Data Structure / Data Sciences
- [데이터 구조 개요](data_structures.md)
- [Pandas](pandas.md)
- [Numpy](numpy.md)
- [Matplotlib](Matplotlib.md)

## 4. Machine Learning
- [Machine Learning Basic](ml_basic.md)
- [머신러닝 학습 방법 비교표](ml_comparison.md)
- [지도학습과 자율주행 샘플 코드](autonomous_driving_ml.md)
- [비지도학습과 자율주행 샘플 코드](unsupervised_learning_guide.md)

## 5. OpenCV
- [OpenCV Basic](OpenCV_basic.md)
- [이미지 처리](image_test.md)

## 6. CNN (Convolution Neural Network)
- [CNN_Basic](CNN_basic.md)
- [CNN_자율주행 관련 코드](cnn_test.md)

## 7. Ultralytics
- [Ultralytics_Basic](Ultralytics_basic.md)
- [Ultralytics_v8](Ultralytics_v8.md)
- [Ultralytics_v11](Ultralytics_v11.md)
- [Ultralytics_v12](Ultralytics_v12.md)

## 8. Deep Learning Frameworks 
- [TensorFlow_basic](TensorFlow_basic.md)
- [TensorFlow_automovil](TensorFlow_automovil.md)
- [PyTorch_Basic](PyTorch_basic.md)
- [TensorFlow vs PyTorch vs TensorRT 비교](frameworks_comparison.md)
- [TensorRT](TensorRT_test.md)

## 9. TAO Toolkit on RunPod
- [TAO_사용법](TAO_install.md)
- [TAO_Toolkit](TAO_Toolkit.md)

## 10. 칼만필터, CARLA, 경로 알고리즘
- [Kalman Filter](kalman.md)
- [CARLA_simulator](CARLA.md)

## 11. ADAS & Advanced Topics
- [ADAS_Basic](adas_basic.md)
- [TensorRT vs PyTorch 비교](vs.md)

---

## 📊 최근 추가된 내용
- 머신러닝 학습 방법 비교 (지도학습, 비지도학습, 강화학습)
- 자율주행에서의 지도학습 활용 예시 코드
- TensorFlow, PyTorch, TensorRT 상세 비교
- Matplotlib 완전 정리 가이드

## 🎯 학습 목표
이 리포지토리는 AI와 자율주행 관련 기술들을 체계적으로 정리하고 실습 코드를 모아둔 개인 학습 노트입니다.

## 🛠️ 주요 기술 스택
- **언어**: Python
- **라이브러리**: TensorFlow, PyTorch, OpenCV, Ultralytics
- **도구**: Jupyter Notebook, Colab, GitHub
- **응용분야**: 컴퓨터 비전, 머신러닝, 자율주행
